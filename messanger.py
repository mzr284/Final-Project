...
engine = create_engine("sqlite:///dzsdfdsfdsfdsfsd.db")
...
class MessengerPanel(QMainWindow):
    ...
    def open_chat(self, contact):
        ...
        for message in self.main_user.message[f"{contact.get_info()['username']}"]:
            if "=" not in message: # if you want sent message:
                ...
                new_label_contact.setStyleSheet("color: white;")
    ...
    def add_message_to_chat(self, message):
        ...
        self.new_msg.setStyleSheet("color: white;")
    ...
    def add_picture_to_chat(self, addr_picture_sender):
        ...
        new_sender_label.setStyleSheet("color: white;")
    ...
    def receive_msg(self):
        while 1:
            ...
            if "=" not in msg: # if sent a message:
                ...
                for user in users:
                    if user.get_info()['username'] == sender:
                        sender_msg = user
                print(f"{sender_msg.get_info()['username']} sent a picture")
                if self.main_user.get_info()['username'] == reciever_msg:
                    self.main_user.message[f"{sender_msg.get_info()['username']}"].append(msg)
